...
def build_asr(args: argparse.Namespace) -> SherpaOnnxRuntimeASR:
    if args.asr_engine != "sherpa_onnx":
        raise SystemExit(f"Unsupported ASR engine for streaming ASR server: {args.asr_engine}")
    return SherpaOnnxRuntimeASR(
...
def normalize_for_noise_match(text: str) -> str:
    return re.sub(r"[^A-Za-z0-9\u4e00-\u9fff]+", "", text).lower()


def should_drop_asr_text(text: str, duration_seconds: float, args: argparse.Namespace) -> tuple[bool, str]:
    stripped = text.strip()
    if args.drop_empty_asr and not stripped:
        return True, "empty_asr_text"
    if duration_seconds < args.min_asr_duration_seconds:
        return True, "short_utterance_duration"
    if visible_text_length(stripped) < args.min_asr_text_chars:
        return True, "short_asr_text"
    if normalize_for_noise_match(stripped) in args.asr_noise_phrases:
        return True, "noise_phrase"
    return False, ""


async def send_asr_filtered(
    websocket: Any,
    utterance_index: int,
    audio_path: str,
    duration_seconds: float,
    ...
    print("ASR_FILTERED:", payload)
    await websocket.send(json.dumps(payload, ensure_ascii=False))


def transcribe_utterance(asr_runtime: SherpaOnnxRuntimeASR, audio_path: str) -> tuple[str, float]:
    start = time.perf_counter()
    text = asr_runtime.transcribe(audio_path)
...
async def process_utterance(
    ...
    await websocket.send(json.dumps(saved_payload, ensure_ascii=False))
    print("UTTERANCE_SAVED:", saved_payload)
...
async def handle_connection(websocket: Any, args: argparse.Namespace, vad: Any, asr_runtime: SherpaOnnxRuntimeASR) -> None:
        ...
        async for message in websocket:
            if isinstance(message, bytes):
                ...
                if utterance is not None:
                    utterance_index += 1
                    accepted = await process_utterance(websocket, args, vad, asr_runtime, utterance, utterance_index)
                    if not accepted:
                        utterance_index -= 1
                continue
            ...
            if message_type == "stream_start":
                ...
                await websocket.send(
                    json.dumps(
                        {
                            "type": "stream_asr_ready",
                            "server_time": time.time(),
                            "vad_engine": vad.name,
                            "asr_engine": args.asr_engine,
                            "message": "Send PCM16 frames; utterances will be transcribed when endpoints are detected.",
                        },
                        ensure_ascii=False,
                    )
                )
                ...
                if streaming and segmenter is not None:
                    ...
                    if utterance is not None:
                        ...
                        if not accepted:
                            utterance_index -= 1
                ack = {
                    "type": "stream_asr_stopped",
                    "server_time": time.time(),
                    "frames_received": frame_count,
                    "utterances_transcribed": utterance_index,
                    "client_stop": payload,
                }
...
async def run_server(args: argparse.Namespace) -> None:
    ...
    async with websockets.serve(handler, args.host, args.port, max_size=None):
        print(f"Auralis stream ASR server listening on ws://{args.host}:{args.port}")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run streaming VAD endpointing and ASR transcription over WebSocket.")
    parser.add_argument("--host", default="0.0.0.0")
    parser.add_argument("--port", type=int, default=8768)
    parser.add_argument("--output-dir", default="outputs/ws_stream_asr_utterances")
    ...
    parser.add_argument(
        "--asr-noise-phrases",
        default=",".join(sorted(DEFAULT_NOISE_PHRASES)),
        help="Comma-separated normalized ASR texts to drop before later LLM/TTS stages.",
    )
